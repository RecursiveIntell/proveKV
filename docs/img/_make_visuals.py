...
Headline N=8 values are reconciled with CLAIMS.json and its bound receipts;
legacy comparison values remain explicit inputs in this generator.
"""
import json
import pathlib
...
# ---------------------------------------------------------------------------
C_FIB = "#7c3aed"      # purple — shared cold tier (FibQuant quantized pool)
...
def normalize_svg(path):
    """Remove generator-only trailing spaces so git diff checks stay clean."""
    text = path.read_text()
    path.write_text("\n".join(line.rstrip() for line in text.splitlines()) + "\n")

# Historical single-pool outputs. Their PPL method is not cache-aligned, so
# these values are displayed only as unadmitted receipt history.
POOL_RUNS = [
...
# Current SmolLM2 N=8 claim. It is used only by the architecture diagram;
# the N-scaling chart remains one internally consistent Qwen0.5B size-only
# series at every N.
CURRENT_CLAIMS = load_json("CLAIMS.json")["claims"]
N8_LOSSLESS = CURRENT_CLAIMS["smollm2_wikitext2_n8_lossless_default"]
...
def make_architecture():
    ...
    ax.text(2.75, 4.15, "fib_k4_n32_batched  ·  codebook quantizer", ha="center", va="center",
            fontsize=10, color=C_MUTED, style="italic")
    ax.text(2.75, 3.65, f"{N8_LOSSLESS['pool_bytes'] / 1024 / 1024:.2f} MiB  ·  blake3-addressed  ·  built once", ha="center", va="center",
            fontsize=10.5, color=C_TEXT, fontweight="bold")
    ax.text(2.75, 3.20, f"pool-only: {N8_LOSSLESS['pool_compression_ratio']:.2f}× vs f32 raw", ha="center", va="center",
            fontsize=9.5, color=C_MUTED)
    ax.text(2.75, 2.85, "shared-prefix PPL checked separately", ha="center", va="center",
            fontsize=9.5, color=C_MUTED, fontweight="bold")
    ...
    for i, (y, lab) in enumerate(zip(shell_y, shell_labels)):
        shell = FancyBboxPatch(
            ...
            linewidth=1.2, edgecolor=C_TURBO, facecolor=C_TURBO + "12", zorder=2,
        )
        ax.add_patch(shell)
        ax.text(8.75, y + 0.20, f"Hot tier  ·  {lab}", ha="center", va="center",
                fontsize=11, fontweight="bold", color=C_TURBO)
        ax.text(8.75, y - 0.20, f"{N8_LOSSLESS['shell_bytes_per_agent'] / 1024 / 1024:.2f} MiB  ·  TurboQuant b=4 (f32 radii)",
    ...
    ax.text(8.75, 0.32, f"{N8_LOSSLESS['raw_total_bytes'] / 1024 / 1024 / 1024:.2f} GiB at N=8", ha="center", va="center",
            fontsize=10, color=C_NAIVE, fontweight="bold")

    # ---- Brace + reduction label on far right ----
    ax.plot([10.85, 10.85], [1.0, 4.85], color=C_TURBO, linewidth=1.6, zorder=2)
    ax.plot([10.80, 10.85], [4.85, 4.85], color=C_TURBO, linewidth=1.6, zorder=2)
    ax.plot([10.80, 10.85], [1.0, 1.0], color=C_TURBO, linewidth=1.6, zorder=2)
    ax.text(11.05, 2.92,
            f"{N8_LOSSLESS['ratio_vs_f32_raw']:.2f}× vs f32-raw\n{N8_LOSSLESS['ratio_vs_fp16_kv']:.2f}× vs fp16\nN=8 size result\n(b=4 f32-radii)",
    ...
    ax.text(0.5, 5.30, "Shared quantized FibQuant pool (built once) + per-agent TurboQuant shells.",
            fontsize=10, color=C_MUTED, style="italic")
    ...
    leg_items = [
        mpatches.Patch(facecolor=C_FIB + "22", edgecolor=C_FIB,
                       label="Shared pool · FibQuant · quantized"),
        mpatches.Patch(facecolor=C_TURBO + "22", edgecolor=C_TURBO,
                       label="Per-agent shell · TurboQuant · f32 radii"),
    ]
    ax.legend(handles=leg_items, loc="lower left", bbox_to_anchor=(0.0, -0.02),
              frameon=False, ncol=2, fontsize=9, handlelength=2.0)

    fig.text(0.5, 0.02,
             f"{N8_LOSSLESS['ratio_vs_f32_raw']:.2f}x is the N=8 size ratio versus eight independent 828-token f32 contexts; "
             f"{N8_LOSSLESS['ratio_vs_fp16_kv']:.2f}x is fp16-equivalent. Shared-prefix continuation: PPL "
             f"{N8_LOSSLESS['oracle_ppl']:.4f} → {N8_LOSSLESS['roundtrip_ppl']:.4f} "
             f"({N8_LOSSLESS['delta_ppl_pct']:+.2f}%); shell K/V unmeasured. See CLAIMS.json.",
             ha="center", va="bottom", fontsize=7.5, color=C_MUTED, style="italic")

    fig.tight_layout()
    output = OUT / "architecture.svg"
...
#   - N=2..8 bars (Qwen0.5B, size-only):
#     results/bench/multi_agent_compact_lossless_lossy/qwen2.5-0.5b/n{N}_{lossless|lossy}/state.json
# Current SmolLM2 N=8 receipts are intentionally not spliced into this Qwen
# series; that mixed-model chart would not have one coherent estimand.
def make_n_scaling():
    ...
    # Every bar belongs to the same Qwen0.5B size-only series.

    # ---- Right panel: proveKV lossless + lossy ----
    # All bars use Qwen0.5B synthetic numbers (size-only).
    ...
    ax_pk.set_title("proveKV two-tier  ·  Qwen0.5B size-only sweep", fontsize=11, pad=14)
    ...
    fig.suptitle("Multi-agent memory scaling at 1024 tokens, 80% shared prefix\n"
                 "Qwen2.5-0.5B synthetic size-only sweep at every N; no PPL result is attached to these bars",
                 fontsize=10, fontweight="bold", y=1.02)
    fig.tight_layout()
    output = OUT / "n_scaling.svg"
    fig.savefig(output, format="svg", bbox_inches="tight")
    normalize_svg(output)
    plt.close(fig)
...
# ---------------------------------------------------------------------------
# 3) Historical single-pool output matrix (PPL values unadmitted)
# ---------------------------------------------------------------------------
def make_cross_validation():
    ...
    ax.set_title("Historical single-pool receipt outputs  ·  PPL values unadmitted (full-cache/full-input method)",
    ...
    ax.legend(handles=[
        mpatches.Patch(facecolor=C_FIB, label="receipt-reported ΔPPL = 0.00% (unadmitted)"),
        mpatches.Patch(facecolor=C_LOSSY, label="receipt-reported ΔPPL < 0% (unadmitted)"),
